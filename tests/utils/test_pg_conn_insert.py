...
# 首先模拟 singleton 装饰器，使其返回类本身而不是单例函数
from unittest.mock import patch
import sys

# 在导入 PostgresConnection 之前，先模拟 singleton 装饰器
mock_singleton = MagicMock()
mock_singleton.side_effect = lambda cls: cls  # 让装饰器返回类本身，而不是单例函数
sys.modules['rag.utils.singleton'] = MagicMock()
sys.modules['rag.utils.singleton'].singleton = mock_singleton

# 现在导入 PostgresConnection
...
class TestPostgresConnectionInsert(unittest.TestCase):
    ...
    def setUp(self):
        """设置测试环境"""
        # 使用 patch 模拟 PostgresConnection 的方法
        patcher1 = patch.object(PostgresConnection, '_create_connection')
        patcher2 = patch.object(PostgresConnection, '_init_database')
        self.mock_create_connection = patcher1.start()
        self.mock_init_database = patcher2.start()
        self.addCleanup(patcher1.stop)
        self.addCleanup(patcher2.stop)
        
        # 初始化 PostgresConnection 实例
    ...
    @patch.object(PostgresConnection, '_get_connection')
